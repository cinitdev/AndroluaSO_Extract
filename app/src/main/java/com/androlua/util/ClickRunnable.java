...
public class ClickRunnable implements Runnable {
    ...
    private boolean postClick(String name) {
        ...
        if (idx > 0) {
            try {
                time = Long.parseLong(name.substring(idx + 1));
            } catch (Exception e) {
                time = 1000;
            }
        ...
        if (idx > 0) {
            if (mN < 0) {
                try {
                    mN = Integer.parseInt(name.substring(idx + 1));
                } catch (Exception e) {
                    mN = -1;
                }
            }
        ...
        if (idx > 0) {
            if (mM < 0) {
                try {
                    mM = Integer.parseInt(name.substring(idx + 1));
                } catch (Exception e) {
                    mM = -1;
                }
            }
    ...
    public void run() {
        ...
        if (obj instanceof LuaTable bs) {
            ...
            mClick.canClick((bool, bs1, name, idx) -> {
                mClick = null;
                run();
            });
